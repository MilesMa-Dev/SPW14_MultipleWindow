...
void main() {
    ...
    float angle = atan(pos.y, pos.x) - info.y * 0.1 * mix(0.5, 1., circlularForce);

    float targetRadius = mix(info.x, 1.8, 0.5 + 0.45 * sin(angle*1. + time*0.2));
    ...
    // pos.xy += vec2(0.001);
    pos.xy += curl(pos.xyz * 7.0, time*1.6, 0.001).xy * 0.002;

    float dist = length(pos.xy - uMouse);
    vec2 dir = normalize(pos.xy - uMouse);
    pos.xy += dir * 0.1 * info.z * smoothstep(0.3, 0.1, dist);
